...
def time_leaking_function(k, BASE=100.0, ALPHA=0.6, BETA=1.1, SIGMA=0.5):
    t = BASE + ALPHA * k.bit_length() + BETA * popcount(k)
    t += random.gauss(0, SIGMA)
    return t

def generate_dataset(n=200, f_small=0.3, b_small=16, seed=56, key_size=1024, 
                    BASE=100.0, ALPHA=0.6, BETA=1.1, SIGMA=0.5):
    ...
    for i in range(n):
        ...
        attack_rec = {
            "index": i,
            "time_taken": time_leaking_function(k, BASE, ALPHA, BETA, SIGMA),
            "H_m": H_m,
            "message": message.hex(),
            "r": r,
            "s": s,
...
def execute_complete_attack(bits=8, timing_method='kmeans', n=200, f_small=0.3, 
                           b_small=16, seed=56, key_size=1024, BASE=100.0, 
                           ALPHA=0.6, BETA=1.1, SIGMA=0.5):
    ...
    records, q, true_private_key = generate_dataset(
        n=n, f_small=f_small, b_small=b_small, seed=seed, key_size=key_size,
        BASE=BASE, ALPHA=ALPHA, BETA=BETA, SIGMA=SIGMA
    )
...
def main():
    ...
    parser.add_argument('--f-small', type=float, default=0.3,
                       help='Fraction of small nonces (default: 0.3)')
    parser.add_argument('--b-small', type=int, default=16,
                       help='Bitlength for small nonces (default: 16)')
